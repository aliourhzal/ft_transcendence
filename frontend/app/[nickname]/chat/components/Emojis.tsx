...
const Emojis:React.FC<EmojisProps> = ( { className, inputRef, setShowEmojies } ) => {
  ...
  return (
      <div className={className + ' px-2 w-[16.1rem] h-[14.8rem] flex flex-col justify-center items-center absolute bottom-[90%] left-0 border-0 border-slate-900 opacity-95 z-20'}>
        <Search _Filter={filerEmojis} type={'emojis'}/>
        <div ref={emojisRef} className='w-full h-full flex flex-wrap flex-col overflow-x-scroll scrollbar-thin scrollbar-track-darken-300 scrollbar-thumb-whiteSmoke scrollbar-corner-black'>
          {_emojis.map( emoji =>
