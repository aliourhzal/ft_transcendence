...
const Notification:React.FC<NotificationProps> = ( { notifications, notify, setNotify, newNotif, setNewNotif } ) => {
  ...
  return (
    <div className='absolute flex flex-col justify-center items-end right-[1%] top-20 h-auto z-10' onBlur={ () => {
      setNotify(false)
    }}>
      ...
      {notify && <div className='rounded-xl gap-1 z-50 flex flex-col items-center justify-center mt-3'>
        { notifications.length ?
          notifications.map(notif =>
             <div className={'w-full bg-darken-300 hover:bg-slate-600 border-2 rounded-xl p-5 flex items-center h-10 text-white'} key={gimmeRandom()}>
              {notif.type === 'good' ? <ImCheckmark color='green' className='m-2'/> : <GiCrossMark color='red' className='m-2'/>}
              {notif.text}
            </div>
          ) : <div className='w-full bg-darken-300 hover:bg-slate-600 border-2 rounded-xl p-5 flex items-center h-10 text-white'>No notifications currently</div>
        }
