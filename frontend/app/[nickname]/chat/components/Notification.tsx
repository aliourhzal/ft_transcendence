...
const Notification:React.FC<NotificationProps> = ( { notifications, notify, setNotify, newNotif, setNewNotif } ) => {
  ...
  return (
    <div>
      {newNotif && <span className='animate-ping absolute inline-flex w-2 h-2 rounded-full bg-blueStrong right-[4%] top-[2%] z-10 opacity-75'></span>}
      ...
      {notify && <div className='rounded-xl gap-1 absolute z-50 right-[4%] top-[6%] flex flex-col items-center justify-center' onBlur={ () => {
        ...
        { notifications.length ?
          notifications.map(notif =>
             <ul className={'w-full bg-darken-300 hover:bg-slate-600 border-2 rounded-xl p-5 flex items-center h-10 text-white'} key={gimmeRandom()}>
