    ...
    <div tabIndex={_tabIndex} className={'transition-all ' + (activeUserConv.id === data.id ? 'bg-blueStrong' : 'bg-zinc-800 hover:bg-zinc-700') + " cursor-pointer convGroup z-0 focus:bg-blueStrong w-[70%] left-[15%] h-[100px] gap-4 relative my-3 rounded-md active:bg-blue-500 flex items-center justify-start text-[16px]"} onClick={handleClick} onFocus={handleClick}>
        ...
        <div className='flex flex-col justify-center gap-2 w-[100%] h-[50%] items-start'>
          ...
          <div className="left-[30%] top-[50%] text-gray-200 text-opacity-70 font-normal text-sm">{
            lastmsg?.msg ? lastmsg?.msg.length > 10 ? (rooms.find(o => o.id === data.id)?.users.find(o => o.id === lastmsg.userId)?.nickName === userData.nickname ? 'you' : (rooms.find(o => o.id === data.id)?.users.find(o => o.id === lastmsg.userId)?.nickName ? rooms.find(o => o.id === data.id)?.users.find(o => o.id === lastmsg.userId)?.nickName : '@')) + " : " + (lastmsg.msg === '%GameInvite%' ? 'gameInvite' : lastmsg?.msg.substring(0, 10) + '...') : (rooms.find(o => o.id === data.id)?.users.find(o => o.id === lastmsg.userId)?.nickName === userData.nickname ? 'you' : (rooms.find(o => o.id === data.id)?.users.find(o => o.id === lastmsg.userId)?.nickName ? rooms.find(o => o.id === data.id)?.users.find(o => o.id === lastmsg.userId)?.nickName : '@')) + " : " + (lastmsg?.msg === 'gameInvite' ? 'pong invite' : lastmsg?.msg) : ''}</div>
        </div>
        { pending &&
