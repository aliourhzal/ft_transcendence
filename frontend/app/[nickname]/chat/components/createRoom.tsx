...
const RoomForm = () => {
    ...
    const handleKeyDown = (e:any) => {
        if (e.key === 'Enter' && roomName != '' && users.length)
            confirmForm()
    }
    ...
    return (
        <Popup isOpen={showForm} modalAppearance={hideForm}>
            ...
            <div className="relative z-0 w-full mb-6 group">
                <input onKeyDown={handleKeyDown} aria-required='true' autoComplete='off' value={roomName} type="text" name="floating_text" id="floating_text" className="text-gray-300 block py-2.5 px-0 w-full text-sm bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required 
            ...
            <div className="relative z-0 w-full mb-6 group">
                <input onKeyDown={handleKeyDown} autoComplete='off' type="text" name="floating_desc" id="floating_desc" className="text-gray-300 block py-2.5 px-0 w-full text-sm bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
            ...
            <button type="button" className="w-auto text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center" onClick={confirmForm} disabled={roomName != '' && users.length ? false : true} >Submit</button>
        </Popup>
    )
}
