  ...
  const OthersChatBox:React.FC<SelfChatBoxProps> = (msg) => {
    return (
        <div className={'z-0 flex items-start justify-start gap-3 m-2 my-4'} key={gimmeRandom()}>
            ...
            <div className="mt-4 break-all max-w-[70%] text-sm font-semibold overflow-hidden py-2 bg-slate-400 rounded-r-3xl rounded-bl-3xl p-5 flex items-center justify-center flex-wrap">{msg.msg}</div>
        </div>
    )
}
